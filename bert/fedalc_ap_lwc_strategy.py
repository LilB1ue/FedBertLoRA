...
class FedALCAPLWCStrategy(FedAvg):
    ...
    # ── Parameter separation (same as FedALC) ──────────────────────────

    def _separate_a_b_others(
        self, parameters: NDArrays
    ) -> Tuple[List[np.ndarray], List[np.ndarray], List[np.ndarray]]:
        """Separate parameters into A, B, and others."""
    ...
    def _compute_layer_scores(
        ...
        """Compute Metric B (dissimilarity × norm) per layer.

        score(l) = (1 - mean_cosine(B_l across clients)) × mean(||B_l||_F)
        """
